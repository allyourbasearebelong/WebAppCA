...
app.use(session({ secret: "topsecret", resave: true,
    saveUninitialized: true })); // Requird to make the session accessable throughout the application


const db = mysql.createConnection({
  host: 'den1.mysql5.gear.host',
  user: 'kennethtest',
  password: 'Ar5D-0518_2j',
  database: 'kennethtest'
...
app.get('/createtable', function(req,res){
 let sql = 'CREATE TABLE plants (Id int NOT NULL AUTO_INCREMENT PRIMARY KEY, Name varchar(255), Price int, Image varchar(255), Variety varchar(255));'
 let query = db.query(sql,(err,res)=>{
  if (err) throw err;
  console.log(res);
  
 });
  res.send("Table Created .... Thanks!")
...
// function to render the lightbox page, this was used while following W3C tutorial on lightbox and a modal gallery
app.get('/lightbox', function(req, res){

  res.render('lightbox', {root: VIEWS});
  console.log("Now you are looking at the lightbox page!");
});
...
app.get('/item/:id', function(req, res){
 ...
 let sql = 'SELECT * FROM plants WHERE Id = "'+req.params.id+'";' 
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
...
app.get('/show/:id', function(req, res){
 ...
 let query = db.query(sql, (err, res1) =>{
  ...
  res.render('show', {root: VIEWS, res1}); // use the render command so that the response object renders a HTML page
  
 });
 
 console.log("Now you are on the show product page!");
});
 



 // function to render the plants page
app.get('/plants', function(req, res){
 
 let sql = 'SELECT * FROM plants WHERE Variety = "Plant";'
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
 
  res.render('plants', {root: VIEWS, res1}); // use the render command so that the response object renders a HTML page
  
 });
  console.log("Now you are on the plants page!");
});
 


  // function to render the cacti page
app.get('/cacti', function(req, res){
 
 let sql = 'SELECT * FROM plants WHERE Variety = "Cacti";'
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
 
  res.render('cacti', {root: VIEWS, res1}); // use the render command so that the response object renders a HTML page
 });
   console.log("Now you are on the cacti page!");
});
  


  // function to render the Succulents page
app.get('/succs', function(req, res){

 let sql = 'SELECT * FROM plants WHERE Variety = "Succulent";'
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
 
  res.render('succs', {root: VIEWS, res1}); // use the render command so that the response object renders a HTML page
  
 });
 console.log("Now you are on the succulents page!");
});

 
app.get('/under10', function(req, res){

 let sql = 'SELECT * FROM plants WHERE price < 10;'
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
 ...
 console.log("Now you are viewing plants that cost less than €10!");
});

app.get('/over10', function(req, res){
...
app.post('/create', function(req, res){
  ...
  let sql = 'INSERT INTO plants (Name, Price, Image, Variety) VALUES ("'+req.body.name+'", '+req.body.price+', "'+req.body.image+'", "'+req.body.variety+'");'
  let query = db.query(sql,(err,res)=>{
  if (err) throw err;
  console.log(res);
  console.log("the Name of the plant is " + name)
 });
...
app.get('/edit/:id', function(req, res){
 ...
 let query = db.query(sql, (err, res1) =>{
  ...
  res.redirect('/login');
  //res.send("You need to log in to edit a product!");
 }
...
let sql = 'UPDATE plants SET Name = "'+req.body.newname+'", Price = "'+req.body.newprice+'", Image = "'+req.body.newimage+'", Variety = "'+req.body.newvariety+'" WHERE Id = "'+req.params.id+'";'
let query = db.query(sql, (err, res) =>{
 if(err) throw err;
 console.log(res);
...
res.redirect("/item/" + req.params.id);
 ...
 // function to delete database data based on button press and form
app.get('/delete/:id', function(req, res){

 let sql = 'DELETE FROM plants WHERE Id = "'+req.params.id+'";'
 let query = db.query(sql, (err, res1) =>{
  if(err)
  throw(err);
 
  res.redirect('/products'); // use the render command so that the response object renders a HTML page
...
// Page to render edit JSON review 
...
app.post('/search', function(req, res){
 ...
 let sql = 'SELECT * FROM plants WHERE name LIKE "%'+req.body.search+'%";'
 let query = db.query(sql, (err,res1) =>{
  if(err)
  throw(err);
 // res.redirect("/error")
...
db.query('INSERT INTO users (Name, Email, Password) VALUES ("'+req.body.name+'", "'+req.body.email+'", "'+req.body.password+'")' 
   ...
   res.send("'+req.body.name+', you're registered now!");
       res.redirect('/');   
});
